	...
	private void handleCompute(ActionEvent event) {
		if (isInputValid()) {
			...
			for(int year : data.keySet()){ //Iterates the Keys
				...
				data.get(year)[3] = rateCow
				data.get(year)[4] = rateDeer
				data.get(year)[5] = rateHorse
				data.get(year)[6] = tempGrass
				 */
				...
				System.out.println(String.format("Year: %d || Food Pop: %s ",year,data.get(year)[6]));
			}
			System.out.println("========================================");
		}
